...
export default function Home() {
  ...
  const handleServiceScroll = () => {
    if (servicesRef.current) {
      ...
      // Adjusted scroll calculation for responsive card widths
      // Mobile card is approx 280px + gap, Desktop is 350px + gap
      const cardWidth = window.innerWidth < 768 ? 300 : 370; 
  ...
  const scrollToService = (index: number) => {
    if (servicesRef.current) {
      const cardWidth = window.innerWidth < 768 ? 300 : 374; 
    ...
    <div className={`${playfair.variable} ${inter.variable} min-h-screen font-sans bg-white text-slate-800`}>
      ...
      {activeService && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white p-6 md:p-12 max-w-lg w-full rounded shadow-2xl relative animate-in zoom-in-95 duration-200 border-4 border-[#C6A87C]">
            <button 
            ...
            <h3 className="text-2xl md:text-3xl font-serif text-slate-900 mb-4">{activeService.title}</h3>
            <p className="text-slate-600 leading-relaxed text-sm md:text-lg mb-8">
              {activeService.text}
            </p>
            <button 
              onClick={() => setActiveService(null)}
              className="bg-[#C6A87C] text-white px-6 py-3 rounded-sm text-xs md:text-sm font-bold uppercase tracking-widest hover:bg-[#a38350] transition w-full md:w-auto"
            >
              Close Details
            </button>
          </div>
      ...
      {/* --- HERO SECTION --- */}
      <section className="relative min-h-[600px] md:h-[750px] w-full bg-black overflow-hidden">
        <div 
        ...
        <div className="relative z-20 h-full max-w-7xl mx-auto px-6 flex flex-col justify-center pt-20">
          <div className="inline-block mb-6">
            <h1 className="text-4xl md:text-8xl font-bold text-white font-serif tracking-tighter drop-shadow-2xl">
          ...
          <p className="text-white text-sm md:text-xl tracking-[0.2em] md:tracking-[0.3em] uppercase font-light drop-shadow-md border-l-4 border-[#C6A87C] pl-4 mb-10">
      ...
      {/* --- WHO WE ARE SECTION --- */}
      <section id="who-we-are" className="relative py-16 md:py-32 bg-white overflow-hidden">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row items-center relative">
            
            {/* Left: Dark Content Box */}
            <div className="bg-[#545454] text-white p-8 md:p-16 md:w-[48%] z-10 shadow-2xl md:-mr-20">
              <h2 className="text-3xl md:text-4xl font-serif mb-6 md:mb-8 border-l-2 border-[#C6A87C] pl-4">Who We Are</h2>
              <p className="text-sm leading-7 md:leading-8 text-gray-300 mb-8 font-light">
              ...
              <div className="space-y-4 font-serif text-base md:text-lg">
            ...
            {/* Right: Office Image */}
            <div className="w-full md:w-[62%] min-h-[300px] md:min-h-[500px] bg-cover bg-center rounded-sm shadow-lg mt-8 md:mt-0"
      ...
      <section className="py-16 md:py-24 bg-slate-50 border-y border-slate-200" id="expertise">
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-8 flex justify-between items-end">
             <div>
               <h2 className="text-3xl md:text-4xl font-serif text-slate-900 mb-2 border-l-4 border-[#C6A87C] pl-6">Our Services</h2>
               <p className="text-slate-500 pl-7 text-xs md:text-sm">Professional solutions for your growth.</p>
             </div>
          </div>
          ...
          >
            {servicesList.map((service) => (
               <div 
                 key={service.id} 
                 className="min-w-[280px] md:min-w-[350px] bg-white p-6 md:p-8 rounded shadow-md border border-slate-100 hover:border-[#C6A87C] transition duration-500 snap-center flex flex-col justify-between group hover:-translate-y-1"
               >
                  <div>
                    <div className="bg-slate-50 p-4 rounded-full w-fit mb-6 group-hover:bg-[#C6A87C]/10 transition">
                       {service.icon}
                    </div>
                    <h3 className="font-bold text-xl md:text-2xl mb-3 font-serif text-slate-800 group-hover:text-[#C6A87C] transition">
                       {service.title}
                    </h3>
                  ...
                  <button 
                    ...
                    className="mt-8 flex items-center gap-2 text-[#C6A87C] text-[10px] md:text-xs font-bold uppercase tracking-widest hover:gap-4 transition-all"
                  >
      ...
      <section className="py-16 md:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <h2 className="text-2xl md:text-3xl font-serif text-slate-900 mb-2">Who We Work For</h2>
          <p className="text-[#C6A87C] font-serif italic mb-10 md:mb-16 text-lg">How We Help</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
            {[
              ...
              <div key={idx} className="bg-[#545454] text-white rounded-sm p-8 flex flex-col items-center justify-center shadow-xl hover:-translate-y-2 transition duration-500 min-h-[200px] group border border-white/10">
      ...
      <section id="insights" className="max-w-7xl mx-auto md:py-24 py-12 px-6 md:px-0">
         <div className="flex flex-col md:flex-row shadow-2xl border border-gray-100">
           ...
           {/* Left: Text */}
           <div className="bg-[#545454] text-white p-8 md:p-16 md:w-1/2 flex flex-col justify-center min-h-[350px] md:min-h-[600px]">
              <h2 className="text-3xl md:text-5xl font-serif mb-3 font-bold">Insights</h2>
              <p className="text-[#C6A87C] mb-8 md:mb-12 font-serif italic text-xl md:text-2xl">Learn, Grow & Succeed</p>
              ...
              <ul className="space-y-6 md:space-y-8">
                {[
                  ...
                  <li key={item} className="flex items-start gap-4">
                     ...
                     <span className="text-lg md:text-xl font-medium text-white tracking-wide">{item}</span>
                  </li>
                ))}
              </ul>
           </div>
           ...
           <div className="md:w-1/2 relative bg-gray-900 min-h-[300px] md:min-h-[600px] overflow-hidden group">
      ...
      {/* --- VISION & MISSION --- */}
      <section className="py-16 md:py-24 px-6 bg-[#1a1a1a] text-white border-t border-white/10">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-8 md:gap-16">
          
          {/* Vision */}
          <div className="bg-[#2a2a2a] p-8 md:p-10 rounded-sm border-l-4 border-[#C6A87C] shadow-xl hover:-translate-y-2 transition duration-500">
            <div className="flex items-center gap-4 mb-6">
              <div className="p-3 bg-[#C6A87C]/10 rounded-full">
                 <Target className="w-8 h-8 text-[#C6A87C]" />
              </div>
              <h2 className="text-2xl md:text-3xl font-serif font-bold">Our Vision</h2>
            </div>
          ...
          <div className="bg-[#2a2a2a] p-8 md:p-10 rounded-sm border-l-4 border-[#C6A87C] shadow-xl hover:-translate-y-2 transition duration-500">
             <div className="flex items-center gap-4 mb-6">
              ...
              <h2 className="text-2xl md:text-3xl font-serif font-bold">Our Mission</h2>
            </div>
            <p className="text-gray-300 leading-relaxed text-base md:text-lg font-light">
